...
export function StoryTemplate(p:Props){const insets=useSafeAreaInsets(),{width,height}=useWindowDimensions(),wide=width>=900&&width>height;const source=getAssetSource(p.image);
 ...
 <View style={s.top}><View><Text style={s.brand}>{HOME_BRAND.title}</Text><Text style={s.subtitle}>{HOME_BRAND.subtitle}</Text></View><Text style={s.motto}>SAME PHYSICS.{'\n'}A BRIGHTER{'\n'}TOMORROW.</Text></View>
 ...
 {p.instruction?<View style={s.hint}><Text style={s.hintLabel}>✦  THE WAY FORWARD</Text><Text style={s.hintText}>{p.instruction}</Text></View>:null}
 {p.progress}<ContinueJourneyButton label={p.action} playIcon={false} onPress={p.onContinue} style={{marginTop:22}}/>
 {p.onSecondary?<Pressable accessibilityRole="button" onPress={p.onSecondary} style={s.secondary}><Text style={s.secondaryText}>{p.secondaryLabel??'RETURN HOME'}</Text></Pressable>:null}
