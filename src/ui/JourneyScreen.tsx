...
import {formatMiles,milesAtWorldEnd,milesTraveled} from '../campaign/journeyMiles';
...
export function JourneyScreen({save,devUnlockAll=false,onSelectLevel,onBack,initialWorldId=null,focusLevel=null}:Props){
 ...
 const traveled=milesTraveled(save.campaign);
 const milesLabel=formatMiles(traveled);
 const milesCopy=homeFound||allClear
  ? t('journeyscreen.miles_home_found',{value1:milesLabel})
  : traveled>0
    ? t('journeyscreen.miles_still_far',{value1:milesLabel})
    : t('journeyscreen.miles_traveled',{value1:milesLabel});
 ...
 <Text style={s.miles}>{active?t('journeyscreen.miles_chapter',{value1:formatMiles(milesAtWorldEnd(active.world.id))}):milesCopy}</Text>
