...
export function createJourneyWorldScene(world:JourneyWorld):THREE.Group {
 ...
 const metal=containmentMetal('floor');metal.color.setHex(world==='sky'?0xa5b4bc:0x52636a);metal.map!.repeat.set(1,1);
 const rock=new THREE.MeshPhongMaterial({color:world==='moon'?0x919598:0x665e62,flatShading:true,shininess:3});
 ...
 const light=new THREE.MeshBasicMaterial({color:world==='network'?0xecc371:world==='homeward'?0x91f5ca:0x82cbd9});
 const crystal=new THREE.MeshPhongMaterial({color:world==='homeward'?0x74baac:0x956ee1,emissive:world==='homeward'?0x174a3a:0x251452,shininess:100});
 ...
 if(world==='sky'){
  ...
  const art=createLateWorldArt(world);root.add(art);
