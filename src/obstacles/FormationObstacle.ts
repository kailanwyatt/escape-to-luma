import {dressMaze} from './EncounterArt';
...
import {FormationConveyorArt} from './FormationConveyorArt';
import {FormationExpandingDebrisArt} from './FormationExpandingDebrisArt';
import {FormationPhaseColumnsArt} from './FormationPhaseColumnsArt';
import {FormationAlternatingDoorsArt} from './FormationAlternatingDoorsArt';

type FormationVariantArt = {
  group: THREE.Group;
...
export class FormationObstacle {
 ...
 private variantArt:FormationVariantArt|null=null;
 ...
 applyConfig(c:FormationConfig,_environment:EnvironmentId){
  ...
  if(c.variant==='conveyor'){
   this.variantArt=new FormationConveyorArt(c);
   this.group.add(this.variantArt.group);
   this.update(0,0);
   return;
  }
  if(c.variant==='expandingDebris'){
   this.variantArt=new FormationExpandingDebrisArt(c);
   this.group.add(this.variantArt.group);
   this.update(0,0);
   return;
  }
  if(c.variant==='phaseColumns'){
   this.variantArt=new FormationPhaseColumnsArt(c);
   this.group.add(this.variantArt.group);
   this.update(0,0);
   return;
  }
  if(c.variant==='alternatingDoors'){
   this.variantArt=new FormationAlternatingDoorsArt(c);
   this.group.add(this.variantArt.group);
   this.update(0,0);
   return;
  }
  ...
  if(c.variant==='rotatingMaze'){
   ...
   // Residual box variants only — all authored formation families have cinematic kits.
   for(const p of formationParts(c,0)){
    const mesh=new THREE.Mesh(new THREE.BoxGeometry(1,1,.2),metal.clone());
 ...
 update(_dt:number,time:number){
  ...
  if(this.variantArt){this.variantArt.update(time);return;}
  ...
  formationParts(c,time).forEach((p,i)=>{
   ...
   m.color.setHex(0x243443);
   const light=this.indicators[i]?.material as THREE.MeshBasicMaterial|undefined;
   light?.color.setHex(p.warning?0xffbd52:0x66edff);
   mesh.visible=p.height>.002;
  });
 }
 hide(){this.active=false;this.group.visible=false;}
 predictState(delta:number,time:number){
  const p=emptyPredictedState('formation',this.z);
  p.angle=time+delta;
  if(this.config?.variant==='rotatingMaze'){
   const a=(time+delta)*this.config.speed*(this.config.direction??1)+(this.config.phase??0);
   const cy=this.config.centerY??3;
   p.openingX=Math.cos(a)*.85;
   p.openingY=cy+Math.sin(a)*.85;
   p.openingRadius=this.config.openingRadius??1.05;
  }
  return p;
 }
 ...
 private disposeVisuals(){
  this.variantArt?.dispose();this.variantArt=null;
