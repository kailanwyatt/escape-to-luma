...
const COURSES:Record<number,{base:number;pace:number;side:number;targetX:number;hint:string;combine?:boolean}>={
 ...
 116:{base:113,pace:1.08,side:-1,targetX:.9,hint:'Two membranes — wait until both fade cyan, then throw through the corridor.'},
 118:{base:113,pace:1.16,side:1,targetX:-1.05,hint:'Quicker membranes — amber means the far field is about to solidify.'},
 ...
 137:{base:113,pace:1.18,side:-1,targetX:1.05,hint:'Familiar membranes — cross both while cyan before they return.'},
...
export function applyEncounterProgression(source:CampaignLevelDefinition):CampaignLevelDefinition {
 ...
 for(const [i,o] of course.challenge.obstacles.entries()){
  if(o.type==='formation'){
   ...
   o.speed*=recipe.pace;
   // Preserve depth cascade after pace: same local at each crossing for mid-power throws.
   o.phase=(-o.z/8.5)*o.speed;
   o.centerX=recipe.targetX*(o.z/12)*0.45;
