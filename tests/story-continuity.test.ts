...
describe('story continuity',()=>{
 ...
 expect(journeyDestinationLabel(['network'],false)).toBe('UNKNOWN');
 expect(journeyDestinationLabel(['network','homeward'],false)).toBe('HOME');
 for(const w of WORLDS.filter(w=>w.lastLevel<135))expect(storyAfterWorld(w.lastLevel,[])!.body).not.toContain('Luma');
 ...
 const s=pendingWorldStory(121,[],progress)!;expect(s.id).toBe('departure.nebula.v1');
