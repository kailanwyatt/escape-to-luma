/**
 * Look-dev showcase: cinematic containment / reactive gate.
 * Uses gameplay ContainmentGateArt + reactiveGateStateAtTime.
 */
...
import type { ReactiveGateConfig } from '../src/config/ObstacleConfig';
...
const CONFIG: ReactiveGateConfig = {
  type: 'reactiveGate',
...
const gate = new ContainmentGateArt(CONFIG);
...
renderer.setAnimationLoop((now) => {
  ...
  status.textContent = `t=${time.toFixed(2)}s · phase ${s.phase.toUpperCase()} · gap ${s.gap.toFixed(2)}`;
  controls.update();
