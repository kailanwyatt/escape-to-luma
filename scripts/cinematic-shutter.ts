/**
 * Look-dev showcase: cinematic split shutter.
 * Uses gameplay FacilityShutterArt + splitShutterStateAtTime.
 */
...
import type { SplitShutterConfig } from '../src/config/ObstacleConfig';
...
const CONFIG: SplitShutterConfig = {
  type: 'splitShutter',
...
camera.position.set(-3.6, 3.2, -8.2);
...
const shutter = new FacilityShutterArt(CONFIG);
shutter.group.traverse((o) => {
...
renderer.setAnimationLoop((now) => {
  ...
  shutter.update(time);
  const s = splitShutterStateAtTime(CONFIG, time);
  status.textContent = `t=${time.toFixed(2)}s · ${s.phase.toUpperCase()} · gap ${s.gap.toFixed(2)}`;
  controls.update();
